...
<div class="features-wrap<?php echo esc_attr($has_borders);?>">
	...
	<?php endif;?>
		<?php foreach ($instance['icon_repeater'] as $feature) :?>
			...
			if($url != '') {
				$icon_title_html = '<a class="'. $title_hover_color .'" href="' . esc_url($url) . '" title="' . esc_html($icon_title) . '><h4 class="item-title ' . $title_hover_color . '">' . $feature['icon_title'].'</h4></a>';		
			} else {
				$icon_title_html = '<h4 class="item-title">' . $feature['icon_title'] . '</h4>';
			};	
