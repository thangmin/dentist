	...
	<?php foreach ($instance['icon_repeater'] as $icon_text) :?>
		...
		if( $icon_text['icon_title']!= '') {
			if($icon_link != '') {
				...
				$icon_title = '<a ' . $target . ' href="' . esc_url($icon_link) . '" title="' . esc_html($icon_text['icon_title']) . '><h4 class="item-title">' . $icon_text['icon_title'].'</h4></a>';		
		
			} else {
				$icon_title = '<h4 class="item-title">' . $icon_text['icon_title'] . '</h4>';
			}
		} 
